...
import { listCookieHostSummaries, setCookiesForHost, deleteCookiesForHost, normalizeCookieHost, looksLikeCookies, YTDLP_COOKIE_MAX_BYTES } from '../utils/ytdlpCookies.js';
    ...
    if (summaries.length === 0) {
        ...
        for (const { host, bytes } of summaries) {
            ...
            rows.push(new Api.KeyboardButtonRow({
                buttons: [new Api.KeyboardButtonCallback({ text: `🗑 ${host}（${size}）`, data: Buffer.from(`ytc_del_${host}`) })],
            }));
        }
    }
    rows.push(new Api.KeyboardButtonRow({
...
async function handleYtdlpCookiesCallback(activeClient: TelegramClient, update: Api.UpdateBotCallbackQuery, data: string): Promise<void> {
    ...
    if (data === 'ytc_close') {
        cookieEntryState.delete(userId);
        await ack('已关闭');
        // 省略 buttons 即可清除内联键盘（传入空 rows 会触发 REPLY_MARKUP_INVALID）
        await activeClient.editMessage(update.peer, { message: update.msgId, text: '🍪 已关闭 Cookie 配置面板。' });
        return;
    }
...
async function handleCookieEntryMessage(message: Api.Message, senderId: number, text: string): Promise<boolean> {
    ...
    // 带媒体的消息（上传的 cookies.txt）交给文件处理器，切勿把空白说明当作 Cookie 文本，
    // 否则会抢先保存空内容并阻止文件下载。
